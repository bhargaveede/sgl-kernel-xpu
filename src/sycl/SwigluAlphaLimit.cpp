...
struct SwigluVec4Kernel {
    ...
    inline void operator()(sycl::nd_item<1> it) const {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            up_v[i]   = sycl::fmax(-gemm1_limit,
                                   sycl::fmin(up_v[i], gemm1_limit));
        }
    ...
    float limit)
{
    ...
    q.submit([&](sycl::handler &h) {
        SwigluVec4Kernel<scalar_t> kernel_functor(
            ...
            limit);
        h.parallel_for(
            sycl::nd_range<1>(global, local),kernel_functor        );
    });
}
...
#define CALL_SWIGLU_VEC4_LAUNCHER_SYCL(scalar_t) \
  {  swiglu_with_alpha_and_limit_sycl<scalar_t>( reinterpret_cast<const scalar_t*>(x_ptr), reinterpret_cast<scalar_t*>(y_ptr),   B,  H, alpha, limit);  }
